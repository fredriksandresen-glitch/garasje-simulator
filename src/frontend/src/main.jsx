...
import { AlertTriangle, Boxes, Gauge, Layers, Package, Ruler, Scale, Settings2, Shield, Warehouse } from "lucide-react";
import "./styles.css";
...
const rooms = [
  { key: "lager1", label: "Lager 1", width: 16.85, usableLength: 22.015, obstructionArea: 4.85 * 1.8 },
  { key: "lager2", label: "Lager 2", width: 16.85, usableLength: 29, extendedLength: 34, obstructionArea: 5.15 * 6.985 }
];
...
function App() {
  ...
  const model = useMemo(() => {
    ...
    const roomModels = rooms.map((room) => {
      const length = room.key === "lager2" && useLager2Extension ? room.extendedLength : room.usableLength;
      const clearWidth = Math.max(0, room.width - wallClearance * 2);
      const clearLength = Math.max(0, length - doorClearance);
      ...
      return { ...room, length, cols, rows, floorSlots, totalSlots: floorSlots * levels, effectiveArea: clearWidth * clearLength - (subtractObstructions ? room.obstructionArea : 0) };
    });
    ...
    return { container, levels, roomModels, totalContainerSlots, totalFootprintArea, totalNeed, kokilleShare, loadRows, requiredContainers, mixedCoverage: totalContainerSlots / Math.max(1, requiredContainers) };
    ...
    <main className="app-shell">
      <section className="dashboard">
        <header className="topbar">
          ...
          <div className={model.mixedCoverage >= 1 ? "score good" : "score warn"}><Gauge size={20} /><span>{Math.round(model.mixedCoverage * 100)}%</span></div>
        </header>
        ...
        <div className="workspace">
          ...
          <section className="stage" aria-label="Lageroversikt">
            <div className="stage-header"><div><h2>Plassmodell</h2><p>{model.container.label}, {model.container.length.toFixed(2)} x {model.container.width.toFixed(2)} x {model.container.height.toFixed(2)} m</p></div><div className="capacity-pill">{formatNumber(model.totalFootprintArea)} m² effektivt areal</div></div>
            <div className="warehouse-map">{model.roomModels.map((room) => <RoomPlan key={room.key} room={room} levels={model.levels} />)}</div>
...
function RoomPlan({ room, levels }) {
  const cells = Math.min(80, room.floorSlots);
  return <article className="room"><div className="room-meta"><div><h3>{room.label}</h3><p>{room.width.toFixed(2)} x {room.length.toFixed(2)} m</p></div><strong>{room.totalSlots} plasser</strong></div><div className="slot-grid" style={{ gridTemplateColumns: `repeat(${Math.max(1, Math.min(room.cols, 10))}, 1fr)` }}>{Array.from({ length: cells }).map((_, index) => <span key={index} className="slot">{levels}</span>)}</div><p className="room-note">{room.rows} rader x {room.cols} lengder, stablet {levels} i høyden.</p></article>;
}
